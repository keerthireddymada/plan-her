...
import { User, Bell, Lock, Palette, HelpCircle, Info, LogOut, ChevronRight, Edit, Save, X } from 'lucide-react';
              ...
              {section.items.map((item, itemIndex) => (
                <motion.div
                  ...
                  className={`px-6 py-4 flex items-center justify-between cursor-pointer hover:bg-black-800 transition-colors ${
                    item.dangerous ? 'hover:bg-red-900' : ''
                  }`}
                ...
                >
                  <div className="flex items-center">
                    <item.icon 
                      size={20} 
                      className={`mr-3 ${item.dangerous ? 'text-red-400' : 'text-lavender-400'}`} 
                    />
                    <span className={`${item.dangerous ? 'text-red-400' : 'text-white'}`}>
                      {item.label}
                    </span>
                  </div>
