...
def test_profile_management(token):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Profile updated: {data['cycle_length']} day cycle")
            return True
        else:
...
def test_predictions(token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Current prediction: {prediction['predicted_mood']} mood on day {prediction['day_of_cycle']}")
        else:
...
def main():
    ...
    print("🔧 Testing PlanHer Backend Core Features (Fresh)")
    print("=" * 50)
